...
async def lifespan(app: FastAPI):
    ...
    if DATA_PATH.exists():
        ...
        ServiceLogger.log_success(f"DashDoor loaded {len(RESTAURANTS)} restaurants", "STARTUP")
    else:
        ServiceLogger.log_warning("No data found in restaurants.json", "STARTUP")
...
def get_restaurant_detail(restaurant_id: str):
    ...
    for r in RESTAURANTS:
        if r["id"] == restaurant_id:
            ServiceLogger.log_success(f"Found restaurant: {r.get('name', 'Unknown')}", "API")
            return r
...
def get_restaurants_by_cuisine(cuisine: str) -> Dict[str, Any]:
    ...
    ServiceLogger.log_info(f"Found {len(filtered)} restaurants for cuisine: {cuisine}", "API")
...
def get_restaurants_by_tags(tags: str) -> Dict[str, Any]:
    ...
    for r in RESTAURANTS:
        ...
        if any(tag in restaurant_tags for tag in tag_list):
            filtered.append(r)
    ServiceLogger.log_info(f"Found {len(filtered)} restaurants matching tags: {tag_list}", "API")
...
def get_menu_items_by_dietary_tags(tags: str) -> Dict[str, Any]:
    ...
    ServiceLogger.log_info(f"Found {len(menu_items)} menu items matching dietary tags: {tag_list}", "API")
