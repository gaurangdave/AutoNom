...
const StatusTab = () => {
  ...
  const checkActiveSessions = useCallback(async () => {
    ...
    try {
      ...
      if (activeSessions && activeSessions.active_sessions_count > 0) {
        const sessionId = activeSessions.session_ids[0];
        
        // If we found a new active session, fetch its state
        if (sessionId !== activeSessionId) {
          ...
          if (sessionState) {
            setStatusTitle('Active Session Found');
            setStatusSubtitle(`Monitoring session: ${sessionId}`);
            setIsActive(true);
          }
        }
      } else {
  ...
  }, [getCurrentUserId, fetchActiveSessionsForUser, fetchSessionState, activeSessionId, setActiveSessionId, setCurrentSessionId]);
  ...
  }, [getCurrentUserId, checkActiveSessions]);
  ...
  const handleModalSubmit = async (response) => {
    ...
    try {
      await submitUserResponse(userId, currentSessionId, response);
      setShowModal(false);
